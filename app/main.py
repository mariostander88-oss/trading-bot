...
async def lifespan(_: FastAPI):
    try:
        ...
        state.notifier = NotificationService(state.settings, state.database, state.broker)
        state.scheduler = create_scheduler(state.bot, state.notifier)
...
def require_ready() -> tuple[Settings, TradingDatabase, AlpacaBroker, TradingBot, NotificationService]:
    if state.config_error:
        raise HTTPException(status_code=503, detail=state.config_error)
    if not state.settings or not state.database or not state.broker or not state.bot or not state.notifier:
        raise HTTPException(status_code=503, detail="Application is not initialized.")
    return state.settings, state.database, state.broker, state.bot, state.notifier
...
def status() -> dict[str, Any]:
    settings, database, _, _, _ = require_ready()
    ...
    return {
        ...
        "daily_goal_blocks_new_buys": settings.daily_goal_blocks_new_buys,
...
def latest_signals(limit: int = 20) -> list[dict[str, Any]]:
    _, database, _, _, _ = require_ready()
...
def run_manual_paper_cycle() -> dict[str, Any]:
    settings, _, _, bot, _ = require_ready()
...
@app.post("/notify/test", dependencies=[Depends(require_api_auth)])
def send_test_notification() -> dict[str, object]:
    _, _, _, _, notifier = require_ready()
    try:
        return notifier.send_status_report(force=True)
